...
if (!(Test-Path -Path "$($completeFile)$step")) {
    ...
    if ($adImageSKU -eq "2008-R2-SP1") {
        # Win2008R2
        "IP address and default gateway configuration for 2008-R2-SP1 (not implemented)" >> $log
    }
    else {
        # Win2012 or above
        "IP address and default gateway configuration for Win2012 or above" >> $log
